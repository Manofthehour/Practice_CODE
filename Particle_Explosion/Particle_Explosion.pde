...
int fadetime = 255; //lower = slower must be between 0-255
...
void setup(){
  smooth();
  gravity = new PVector (0,0.03); // gravity given a value
  size(1440, 900);
  background(0);
  stroke(255, 100, 0);
...
class particle{ // class of a particle object
  ...
  loc = new PVector (width/2,height/2); // random within box
  vel = new PVector (random(-3, 7), random(-5, 5)); //random between 0-maxrandvel
  }
